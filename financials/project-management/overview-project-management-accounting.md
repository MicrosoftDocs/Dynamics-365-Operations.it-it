...
Un progetto è un gruppo di attività che è progettato per consentire un servizio, per ottenere un prodotto, o per ottenere il risultato. I progetti consumano le risorse e la generazione dei risultati finanziari sotto forma di ricavi o di cespiti.

## <a name="projects-across-industries"></a>Progetti tra settori
La funzionalità gestione progetti e contabilità può essere utilizzata in più settori, come illustrato nella figura seguente. [progetti![mediante settori (]. /media/projects-accross-industries.jpg)](. /media/projects-accross-industries.jpg) 

In un call center un ticket può essere utilizzato per descrivere il set di azioni necessarie per risolvere una chiamata. Le società di consulenza, ad esempio la gestione o organizzazioni o agenzie di pubblicità di consulenza tecniche, fare riferimento alle attività come progetti. Nel marketing, una campagna rappresenta un insieme di lavoro da consegnare. Nella produzione basata su progetti, un ordine di produzione si riferisce ai vari lavori che devono essere eseguiti per produrre alcuni prodotti finiti. Questo nome viene utilizzato per determinati, questi progetti sono incluse risorse, le programmazioni e i costi e funzionalità Gestione progetti e contabilità in Microsoft Dynamics 365 per le operazioni può ottenere ulteriori informazioni sulla pianificazione, l'esecuzione e l'analisi dei progetti.
...
Durante l'avvio di progetto, più processi chiave si verificano. È possibile utilizzare un'offerta di progetto per comunicare la manodopera, le spese e i materiali stimati al cliente. È possibile registrare le condizioni di fatturazione, limitare e gli accordi in un contratto di progetto. È possibile utilizzare una struttura di suddivisione del lavoro (WBS) per progettare e stimare il lavoro. È possibile impostare le previsioni di budget e per consentire l'esecuzione di progetto. Nella seguente figura è illustrata la struttura di un progetto. [struttura di progetto![(]. /media/project-structure1.jpg)](. /media/project-structure1.jpg)  
...
| A prezzi fissi       | Nei progetti a prezzo fisso, le fatture sono le transazioni di acconto. Un progetto a prezzo fisso viene fatturato a un programma di fatturazione in base a un contratto di progetto. I ricavi per un progetto a prezzo fisso possono essere calcolati e registrati in tutto il progetto utilizzando il metodo per percentuale completata. In alternativa, i ricavi possono essere calcolati e registrati quando il progetto viene completato, utilizzando il metodo per conclusione dei contratti. Le società possono trarre giovamento spesso da utilizzare il valore di lavoro in (WIP) processo per calcolare il grado di completamento di un progetto o di un gruppo di progetti.                                                                                                                                     |
| Investimento        | I progetti di investimento sono progetti che non determinano profitti immediati. In genere sono utilizzati per i progetti interni a lungo termine dove i costi devono essere capitalizzati. Solo i costi per articoli, ore e spese possono essere registrati per un progetto di investimento. I costi di un progetto di investimento vengono monitorati e controllati dalla funzionalità di stima. È possibile impostare i progetti di investimento con un limite di capitalizzazione massima facoltativo. Mentre un progetto di investimento avanza, si registrano i relativi costi nei conti WIP, dove i costi sono mantenuti fino al completamento del progetto. Quando il progetto viene eliminato, trasferire il valore WIP a un cespite, in un conto CoGe o un nuovo progetto.
> [!NOTE] 
> Le transazioni relative ai progetti di investimento non verranno visualizzate ** Registra costi, ** ** accumula i ricavi **, o ** creare proposte di fatturazione ** nella pagina.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 _=_ _=_ Costi il progetto      _=_ Per i progetti di investimento, i progetti di costi vengono utilizzati in genere tenere traccia dei progetti interni e solo le ore, spese e articoli possono essere registrati la merce. Tuttavia, i piani dei costi hanno in genere una durata inferiore rispetto ai progetti di investimento. Inoltre, a differenza dei progetti di investimento, i piani dei costi non possono essere capitalizzati nei conti dello stato patrimoniale. Le transazioni di progetto vengono invece registrate solo nei conti profitti e perdite. 
[!NOTE] 
> Le transazioni relative ai progetti di costi non vengono riflesse ** Registra costi, ** ** accumula i ricavi **, o ** creare proposte di fatturazione ** nella pagina. Poiché i progetti di costi vengono utilizzati in genere tenere traccia dei progetti interni, in genere non devono essere associati a un conto cliente. Tuttavia, se l'impostazione richiede che le richieste articoli vengano create per gli ordini acquisto, è necessario associare il piano dei costi a un cliente. Questa associazione è necessaria, perché le richieste articoli vengono gestite come righe ordine cliente e il sistema richiede che venga specificato un cliente. Tuttavia, questa impostazione non finirà per creare automaticamente delle richieste articoli da un ordine acquisto. Per i piani dei costi, l'impostazione **Creare la richiesta articolo** è trascurata. Se è necessaria una richiesta articoli in un piano dei costi, è possibile crearne una manualmente, purché un cliente sia associato al progetto. _=_ _=_ Interno          _=_ I progetti interni sono utilizzati tenere traccia dei costi di un progetto per il quale è una persona interna all'organizzazione. I progetti interni possono immettere uno strumento di programmazione per gestire il consumo delle risorse. ** Nota: ** Le transazioni relative ai progetti interni non vengono riflesse ** accumula i ricavi ** o ** creare proposte di fatturazione ** nella pagina.                                                                                                                                                                                                                                                                       _=_ _=_ Ora              _=_ I progetti di tempistica vengono utilizzati tenere traccia del tempo associato alle attività non addebitabili e non produttive, ad esempio un progetto tenere traccia del tempo malato per i lavoratori. Le transazioni nei progetti di tempistica non vengono registrate nella contabilità generale. Sono invece nei report relativi agli utilizzi dei lavoratori. In questi progetti è possibile registrare solo le transazioni orarie. Si utilizza un giornale di registrazione ore o una scheda attività per registrare le ore del progetto. Dopo le ore vengono registrati, questi vengono visualizzate come transazioni di progetto ma non contengono transazioni corrispondenti di giustificativo.

> [!NOTE] 
> Le transazioni relative ai progetti di tempistica non vengono riflesse ** Registra costi, ** ** accumula i ricavi **, o ** creare proposte di fatturazione ** nella pagina.                                                                                                                                                                                       |
...
Di seguito sono riportati alcuni dei modi che è possibile utilizzare la funzionalità di programmazione risorse:
...
-   Articolo consumato/nessun collegamento **** per ordinare il **** - collegare il progetto a una richiesta di articoli. Utilizzando questo metodo, è possibile visualizzare i costi effettivi del progetto quando lo stato della produzione è **Avviato** o finito. I costi vengono registrati in più transazioni articolo di progetto per le materie prime e le ore consumate nella produzione. È inoltre possibile definire il livello della gerarchia DBA in cui viene tenuta traccia dei progetti nella produzione.
...
-   Per ordinare gli articoli dall'inventario della società, è necessario immettere l'ordine come richiesta articolo. Se si utilizza ** richieste articoli ** la pagina, è possibile impostare il fabbisogno il ricevimento di articoli in base a consegne parziali. Di conseguenza, poter posticipare il consumo di una determinata quantità di articoli finché questi non risultano necessari.
...
Microsoft Dynamics 365 per le operazioni non riduce automaticamente le stime programmate. Pertanto, si consiglia di conservare un modello previsionale originale del progetto a prezzo fisso per creare un riferimento di base con cui eseguire il confronto una volta che il progetto è stato completato. 
> [!NOTE] quando si seleziona il metodo, utilizza almeno due modelli previsionali. Un modello deve contenere la previsione originale. Nell'altro occorre copiare le transazioni previsionali da un altro modello. Questo metodo vale solo per i progetti a prezzo fisso e i progetti di investimento.</td>
> </tr>
<tr class="odd">
...
-   ** Data fattura: ** 31 marzo 2012. La data viene determinata in base a frequenza di fatturazione. Per questo esempio si imposterà la frequenza di fatturazione sul mese corrente. Di conseguenza, tutte le transazioni registrate in marzo verranno fatturate l'ultimo giorno del mese.
-   ** Data di scadenza: ** 14 aprile 2012. La data viene determinata in base ai termini di pagamento impostati per il progetto. Per questo esempio sono stati selezionati termini di pagamento di 14 giorni. Di conseguenza, alla data della fattura vengono aggiunti 14 giorni per arrivare alla data di scadenza del 14 aprile 2012.
-   ** Data prevista pagamento vendita: ** 27 aprile 2012. Tale data viene calcolata aggiungendo il numero di giorni ** giorni di tolleranza generali ** nel campo ** gestione progetti e parametri di stima ** alla pagina in numero di giorni ** singoli giorni di tolleranza ** il campo della pagina sopra ** contratti di progetto ** quindi aggiungendo il totale al numero di giorni ** ** scadenza nel campo. Per questo esempio, verrà immesso **3** nel campo **Giorni di tolleranza generali** e **10** nel campo **Giorni di tolleranza particolare**. Di conseguenza, alla data di scadenza vengono aggiunti 13 giorni per arrivare alla data prevista di pagamento vendita del 27 aprile 2012.
...
-   ** Data di scadenza: ** 16 marzo 2012. I termini di pagamento vengono impostati su 14 giorni.
-   ** Data prevista pagamento vendita: ** 29 marzo 2012. Il calcolo sono inclusi i tre giorni di tolleranza generali e i singoli 10 giorni di tolleranza.
...
> [!NOTE] 
> Data di scadenza dell'ordine fornitore si basa sulla transazione fornitore quando l'ordine fornitore del progetto viene creato. La data di scadenza non è determinata da alcuna impostazione predefinita. 
...
> [!NOTE] 
> Quando si utilizza ** controllo costi ** la pagina per visualizzare lo stato corrente dei costi di progetto, utilizzare i modelli previsionali che sono stati prelevati per la fattura originale e il budget residuo. Se si selezionano altri modelli previsionali durante l'esecuzione del calcolo dei costi, i risultati non saranno accurati.
