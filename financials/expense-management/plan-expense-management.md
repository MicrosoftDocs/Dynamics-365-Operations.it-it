...
-   Regole relative alla trasferta giornaliera predefinite per il primo e l'ultimo giorno:
    ...
    -   Esiste una riduzione dell'importo che viene offerto per il vitto per il primo e l'ultimo giorno? In tal caso, qual è la percentuale di riduzione?
    -   Esiste una riduzione dell'importo che viene offerto per l'alloggio per il primo e l'ultimo giorno? In tal caso, qual è la percentuale di riduzione?
    -   Esiste una riduzione dell'importo che viene offerto per le altre spese sostenute il primo e l'ultimo giorno? In tal caso, qual è la percentuale di riduzione?
-   Regole relative alla trasferta giornaliera predefinite:
    -   Esiste una riduzione percentuale di indennità di trasferta giornaliera per ogni pasto se, ad esempio, il pasto è extra? In tal caso, qual è la percentuale di riduzione per ogni pasto?
...
-   Regole relative alla trasferta giornaliera in base all'ubicazione:
    -   Le tariffe della trasferta giornaliera variano in base alle ubicazioni? E quali ubicazioni sono incluse?
    -   Se per la tariffa della trasferta giornaliera varia in base all'ubicazione, per ciascuna ubicazione, quale importo percentuale viene fornito per:
        -   vitto
        -   hotel
        -   altre spese
...
La gestione delle spese richiede che vengano utilizzati più giornali di registrazione e conti. È possibile, ad esempio, decidere se lo stesso conto viene utilizzato per gli anticipi contanti e le controversie su carta di credito. **Decisioni:**
...
Quando i dipendenti creano una nota spese, ogni spesa registrata deve essere associata a una categoria di spesa. Le categorie di spesa derivano dalle categorie condivise che possono essere condivise per tutte le persone giuridiche dell'organizzazione. Tali categorie possono inoltre essere condivise in Gestione progetti e contabilità, in base a come è definita l'organizzazione. In base alla definizione dell'organizzazione e alle istruzioni del team di implementazione, determinare se le categorie utilizzate nella gestione delle spese devono essere utilizzate solo nella spesa o se devono essere condivise tra il progetto e la spesa. Si noti che le categorie possono essere condivise tra progetto e spesa o progetto e produzione, ma non tra spesa e produzione. È necessario prendere le seguenti decisioni per ciascuna categoria di spesa. **Decisioni:**

-   Qual è la categoria di spesa? Ad esempio, volo, hotel o chilometraggio.
...
-   Le spese in questa categoria devono essere dettagliate?
...
-   Sono consentiti metodi di pagamento aggiuntivi per la categoria di spesa? In tal caso, quali sono?
-   Sono presenti sottocategorie all'interno della categoria di spesa? In tal caso:
    -   Le sottocategorie sono escluse dal recupero imposte?
    -   Qual è la fascia IVA articoli relativa alle sottocategorie?

    Se la categoria di spesa viene utilizzata anche in Gestione progetti e contabilità, rispondere alle domande rimanenti. In caso contrario, questa sezione è terminata.
-   Quali conti relativi ai costi verranno utilizzati per quanto segue?
    -   Costo
...
Per le imposte di spesa, è necessario determinare cosa è incluso o abilitato nelle note spese. **Decisioni:**

-   L'IVA è inclusa negli importi relativi alle spese?
-   Il recupero imposte deve essere abilitato sulle spese?

Si noti che, se durante la pianificazione della contabilità generale, si è deciso di applicare regole imposta di utilizzo e imposte sulle vendite USA, impostando il campo  **Applica regole di tassazione IVA** su Sì, non è possibile abilitare il recupero imposte sulle spese.

## <a name="policies"></a>Criteri
È possibile creare criteri delle note spese affinché l'organizzazione possa risparmiare tempo e denaro quando i dipendenti sostengono spese per suo conto. I criteri assicurano che i dipendenti rimangano all'interno del budget, forniscano tutte le informazioni necessarie e spendano denaro solo se necessario. È necessario prendere le seguenti decisioni per tutti i criteri delle note spese e tutti i criteri di approvazione della note spese creati. **Decisioni:**
