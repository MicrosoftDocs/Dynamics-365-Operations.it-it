...
I seguenti parametri influiscono sulla modalità di gestione delle liquidazioni vengono elaborate in Microsoft Dynamics 365 per le operazioni. La liquidazione è il processo di facilitazione di una fattura rispetto a un pagamento o una nota di accredito. Questi parametri sono situati nell'area **Liquidazione** delle pagine **Parametri contabilità clienti** e **Parametri contabilità fornitori**.
...
-   ** Applicazione sconto di cassa ** di controllo a [sconto di cassa viene gestito se la fattura viene pagata in più cash-discount-handling-overpayments.md] (). Per un'eccedenza di pagamento, lo sconto di cassa può essere ridotto, può essere gestito come differenza, oppure può rimanere acces del fornitore o il cliente.
...
-   **Calcola sconti di cassa per note di accredito**: impostare questa opzione su **Sì** per calcolare automaticamente uno sconto di cassa per le note di accredito. In Contabilità clienti, una transazione di nota di accredito è una transazione negativa con un valore nel campo **Fattura** nella pagina **Fattura a testo libero** o un reso nella pagina **Ordine cliente**.
    -   L'effetto di questa opzione dipende dal valore del campo **Utilizzare lo sconto di cassa** nella pagina **Liquida transazioni**. Se l'opzione è impostata su Sì ** **, lo sconto viene prelevato quando il campo del **** dello sconto di cassa di utilizzo del **** è impostato su ** ** normale. Quando il campo del **** dello sconto di cassa di utilizzo del **** è impostato su ** sempre **, lo sconto di cassa verrà sempre prelevato, indipendentemente dall'impostazione di questo campo. Quando il campo del **** dello sconto di cassa di utilizzo del **** è impostato su ** mai **, lo sconto di cassa non verrà mai prelevato, indipendentemente dall'impostazione di questo campo.
...
-   **Assegna priorità a liquidazione (solo Contabilità clienti)**: impostare questa opzione su **Sì** per abilitare il pulsante **Contrassegna in base a priorità** nelle pagine **Pagamenti cliente** e **Liquida transazioni**. Il pulsante consente agli utenti di assegnare l'ordine di liquidazione predeterminato alle transazioni.  Dopo che l'ordine di liquidazione applicato a una transazione, l'ordine e l'allocazione di pagamento possono essere modificati prima della registrazione.
-   ** Priorità di utilizzo delle liquidazioni automatiche ** - impostare questa opzione su Sì ** ** per utilizzare l'ordine di priorità definito quando le transazioni vengono liquidate automaticamente. Questo campo è disponibile solo se ** assegna priorità di liquidazione e ** ** liquidazione automatica ** le opzioni sono impostate su Sì ** **.
