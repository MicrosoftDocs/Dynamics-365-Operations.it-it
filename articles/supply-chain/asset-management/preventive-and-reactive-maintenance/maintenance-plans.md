...
description: In questo articolo vengono descritti i piani di manutenzione in Gestione cespiti.
...
- Un nuovo tipo di intervallo del piano di manutenzione denominato *Ripetuto sul valore aggregato (solo contatore)*. Questo tipo attiva la manutenzione ogni volta che un contatore aggregato raggiunge un multiplo di un valore specifico. Ad esempio, la manutenzione può essere attivata ogni 10.000 ore. Per ulteriori informazioni, vedi la sezione [Panoramica dei tipi di intervallo](#interval-types) prima in questo articolo.
