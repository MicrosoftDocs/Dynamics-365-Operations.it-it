...
I clienti Dynamics 365 Commerce utilizzano l'applicazione POS del produttore oltre che applicazioni POS di terze parti. Con l'applicazione POS del produttore, le transazioni vengono inserite in Headquarters dai canali mediante un processo batch. Con le applicazioni di terze parti, le transazioni vengono inserite in Headquarters tramite integrazione. In entrambi i casi, dopo l'inserimento delle transazioni in Headquarters, è necessario effettuare un processo di verifica della coerenza che esegue diverse convalide sulle transazioni, in modo che solo le transazioni convalidate correttamente vengano inserite nel rendiconto da registrare in Headquarters. 

Le transazioni di commercio non superano la convalida per diversi motivi. Ad esempio, un errore nel codice di integrazione o un errore nell'applicazione POS può causare dati incoerenti; un errore dell'utente, come l'eliminazione di un prodotto dopo la sincronizzazione con il canale o la chiusura di un periodo fiscale senza registrare le transazioni per il periodo, può causare dati incoerenti.

Anche se queste transazioni vengono contrassegnate ed escluse dai rendiconti, possono interrompere il processo di registrazione quotidiana delle vendite nella contabilità finanziaria di un cliente.

In Commerce, è possibile modificare le specifiche transazioni che non superano la convalida e allo stesso tempo gestire il controllo di tutte le modifiche. 
...
2. Passare all'area di lavoro **Dati finanziari punto vendita**. Il riquadro **Errori di convalida transazioni** fornisce una visualizzazione pre-filtrata del modulo della transazione che non ha soddisfatto una o più regole di convalida.
...
4. Nel file di Excel, modificare i campi appropriati, quindi caricare i dati nuovamente in Headquarters utilizzando la funzionalità di pubblicazione del componente aggiuntivo Dynamics Excel. Una volta pubblicate, le modifiche verranno riflesse nel sistema. Durante la pubblicazione non viene effettuata alcuna convalida delle modifiche apportate dagli utenti.
...
In Retail versione 10.0.6 e successive è supportata l'opzione di modificare in blocco le transazioni a livello di rendiconto. 
