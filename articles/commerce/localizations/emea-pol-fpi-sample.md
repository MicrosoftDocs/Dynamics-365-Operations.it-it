...
1. Scarica i file di configurazione per il provider di documenti fiscali e il connettore fiscale:
    ...
    1. Scarica il file di configurazione del provider di documenti fiscali in **CommerceRuntime \> DocumentProvider.PosnetSample \> Configuration \> DocumentProviderPosnetSample.xml**.
    1. Scarica il file di configurazione del connettore fiscale in **HardwareStation \> ThermalDeviceSample \> Configuration \> ConnectorPosnetThermalFVEJ.xml**.
...
> [!NOTE]
> - L'esempio di integrazione della stampante fiscale per la Polonia è disponibile in Commerce SDK a partire dalla versione 10.0.29 di Commerce. In Commerce versione 10.0.28 o precedente, devi utilizzare la versione precedente di Retail SDK in una macchina virtuale per sviluppatori in LCS. Per ulteriori informazioni, vedi [Linee guida per la distribuzione per l'esempio di integrazione della stampante fiscale per la Polonia (legacy)](emea-pol-fpi-sample-sdk.md).
