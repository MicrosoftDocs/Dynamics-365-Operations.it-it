...
### <a name="enable-features-for-austria"></a>Abilitare le funzionalità per l'Austria

Devi abilitare le seguenti funzionalità nell'area di lavoro **Gestione funzionalità**:

- (Austria) Abilita eventi di controllo aggiuntivi in POS
- (Austria) Abilita informazioni aggiuntive nei rendiconti di fine giornata in POS
...
L'esempio di integrazione del servizio di registrazione fiscale per l'Austria si basa sulla [funzionalità di integrazione fiscale](fiscal-integration-for-retail-channel.md) e fa parte di Commerce SDK. L'esempio si trova nella cartella **src\\FiscalIntegration\\Efr** del repository [Soluzioni Dynamics 365 Commerce](https://github.com/microsoft/Dynamics365Commerce.Solutions/). L'[esempio](fiscal-integration-for-retail-channel.md#fiscal-registration-process-and-fiscal-integration-samples-for-fiscal-devices-and-services) consiste di un provider di documenti fiscali, che è un'estensione di Commerce Runtime (CRT), e di un connettore fiscale, che è un'estensione di Commerce Hardware Station. Per ulteriori informazioni su come utilizzare Commerce SDK, vedi [Scaricare esempi di Commerce SDK e pacchetti di riferimento da GitHub e NuGet](../dev-itpro/retail-sdk/sdk-github.md) e [Configurare una pipeline di compilazione per SDK a pacchetti indipendenti](../dev-itpro/build-pipeline.md).

> [!NOTE]
> L'esempio di integrazione del servizio di registrazione fiscale per l'Austria è disponibile in Commerce SDK a partire dalla versione 10.0.29 di Commerce. In Commerce versione 10.0.28 o precedente, devi utilizzare la versione precedente di Retail SDK in una macchina virtuale per sviluppatori (VM) in Microsoft Dynamics Lifecycle Services (LCS). Per ulteriori informazioni, vedi [Linee guida per la distribuzione per l'esempio di integrazione fiscale per l'Austria (legacy)](emea-aut-fi-sample-sdk.md).
...
1. Scarica i file di configurazione per il provider di documenti fiscali e il connettore fiscale:
    ...
    1. Apri **Configurazioni \> DocumentProviders** e scarica i file di configurazione per il provider di documenti fiscali: 
...
> [!NOTE]
> - L'esempio di integrazione del servizio di registrazione fiscale per l'Austria è disponibile in Commerce SDK a partire dalla versione 10.0.29 di Commerce. In Commerce versione 10.0.28 o precedente, devi utilizzare la versione precedente di Retail SDK in una macchina virtuale per sviluppatori in LCS. Per ulteriori informazioni, vedi [Linee guida per la distribuzione per l'esempio di integrazione fiscale per l'Austria (legacy)](emea-aut-fi-sample-sdk.md).
...
L'esempio di integrazione del servizio di registrazione fiscale per l'Austria si basa sulla [funzionalità di integrazione fiscale](fiscal-integration-for-retail-channel.md) e fa parte di Commerce SDK. L'esempio si trova nella cartella **src\\FiscalIntegration\\Efr** del repository [Soluzioni Dynamics 365 Commerce](https://github.com/microsoft/Dynamics365Commerce.Solutions/). L'esempio [consiste](fiscal-integration-for-retail-channel.md#fiscal-registration-process-and-fiscal-integration-samples-for-fiscal-devices-and-services) di un provider di documenti fiscali, che è un'estensione di CRT e un connettore fiscale, che è un'estensione di Commerce Hardware Station. Per ulteriori informazioni su come utilizzare Commerce SDK, vedi [Scaricare esempi di Commerce SDK e pacchetti di riferimento da GitHub e NuGet](../dev-itpro/retail-sdk/retail-sdk-overview.md) e [Configurare una pipeline di compilazione per SDK a pacchetti indipendenti](../dev-itpro/build-pipeline.md).
