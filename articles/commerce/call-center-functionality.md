...
description: Questo articolo fornisce una panoramica delle funzionalità di vendita del servizio clienti in Dynamics 365 Commerce.
author: josaw1
ms.date: 04/03/2018
ms.topic: article
ms.prod: ''
...
In Dynamics 365 Commerce, un servizio clienti è un tipo di canale di commercio che può essere definito nell'applicazione. La definizione di uno specifico canale per le entità del servizio clienti consente al sistema di associare specifici valori predefiniti di dati e di elaborazione di ordini a ordini cliente creati da un utente del canale servizio clienti.

Le funzionalità del servizio clienti includono prezzi e promozioni avanzati, cataloghi, gift card e buoni sconto. Gli ordini del servizio clienti sono utilizzati dall'applicazione POS per supportare scenari di adempimento ordini multicanale.

È importante notare che sebbene il modulo servizio clienti possa essere utilizzato da altri settori oltre a quello del commercio, la versione corrente dell'applicazione servizio clienti non è stata ottimizzata per l'utilizzo in scenari di elaborazione ordini B2B o scenari in cui gli ordini hanno un gran numero di righe di vendita. Si consiglia agli utenti che desiderano utilizzare le funzionalità del servizio clienti per l'elaborazione di ordini al di fuori di una tipica elaborazione di transazioni direct-to-consumer di verificare attentamente che l'attivazione della funzionalità di servizio clienti soddisferà le esigenze prestazionali e funzionali.
...
In questa pagina vengono forniti collegamenti alla documentazione relativi a impostazione, configurazione e utilizzo delle funzionalità del servizio clienti.


## <a name="configure-the-call-center"></a>Configurazione del servizio clienti

[Impostare canali servizio clienti](set-up-order-processing-options.md)
