...
È necessario creare una nuova configurazione ER che contenga un componente modello di dati per il dominio aziendale **Gestione questionari**. Questo modello di dati verrà successivamente utilizzato come origine dati quando si progetta un formato ER per generare il report **Gestione questionari**.
