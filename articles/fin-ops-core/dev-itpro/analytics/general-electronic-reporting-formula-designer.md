...
La figura seguente mostra la progettazione di un'espressione di questo tipo. In questo esempio, l'espressione arrotonda a due posizioni decimali il valore del campo **Intrastat.AmountMST** della tabella Intrastat e quindi restituisce il valore arrotondato.
...
Se l'origine dati **System** viene aggiunta a un mapping ER che fa riferimento alla classe dell'applicazione **Global**, l'espressione restituisce il valore booleano, **FALSE**. L'espressione modificata **System.' isLanguageRTL'("AR")** restituisce il valore booleano **TRUE**.
...
| NOW () | Restituisce la data e l'ora del server applicazioni corrente come valore di data/ora. | |
| TODAY () | Restituisce la data del server applicazioni corrente come valore di data. | |
| NULLDATE () | Restituisce un valore di data **null**. | |
| NULLDATETIME () | Restituisce un valore di data/ora **null**. | |
| DATETIMEFORMAT (data/ora, formato) | Converte il valore specificato di data/ora in una stringa nel formato specificato. (Per informazioni sui formati supportati, vedere [standard](https://msdn.microsoft.com/library/az4se3k1(v=vs.110).aspx) e [personalizzato](https://msdn.microsoft.com/library/8kb3ddd4(v=vs.110).aspx).) | **DATETIMEFORMAT (NOW(), "dd-MM-yyyy")** restituisce la data del server di applicazioni, 24 dicembre 2015, come **"24-12-2015"**, in base al formato specificato personalizzato. |
| DATETIMEFORMAT (data/ora, impostazioni cultura) | Converte il valore di data/ora specificato in una stringa nel formato e nelle [impostazioni cultura](https://msdn.microsoft.com/goglobal/bb896001.aspx) specificati. (Per informazioni sui formati supportati, vedere [standard](https://msdn.microsoft.com/library/az4se3k1(v=vs.110).aspx) e [personalizzato](https://msdn.microsoft.com/library/8kb3ddd4(v=vs.110).aspx).) | **DATETIMEFORMAT (NOW(), "d", "de")** restituisce la data del server applicazioni corrente, 24 dicembre 2015, come **"24.12.2015"**, in base alle impostazioni cultura tedesche selezionate. |
| SESSIONTODAY () | Restituisce la data della sessione corrente dell'applicazione come valore di data. | |
| SESSIONNOW () | Restituisce la data e l'ora della sessione corrente dell'applicazione come valore di data/ora. | |
| DATEFORMAT (data, formato) | Restituisce una rappresentazione di stringa della data specificata nel formato specificato. | **DATEFORMAT (SESSIONTODAY (), "dd-MM-yyyy")** restituisce la data della sessione dell'applicazione, 24 dicembre 2015, come **"24-12-2015"**, in base al formato specificato personalizzato. |
| DATEFORMAT (data, formato, impostazioni cultura) | Convertire il valore di data specificato in una stringa nel formato e nelle [impostazioni cultura](https://msdn.microsoft.com/goglobal/bb896001.aspx) specificati. (Per informazioni sui formati supportati, vedere [standard](https://msdn.microsoft.com/library/az4se3k1(v=vs.110).aspx) e [personalizzato](https://msdn.microsoft.com/library/8kb3ddd4(v=vs.110).aspx).) | **DATETIMEFORMAT (SESSIONNOW (), "d", "de")** restituisce la data della sessione corrente dell'applicazione, 24 dicembre 2015, come **"24.12.2015"**, in base alle impostazioni cultura tedesche selezionate. |
...
| DAYS (data 1, data 2) | Restituisce il numero di giorni compresi tra la prima data specificata e la seconda data specificata. Restituisce un valore positivo quando la prima data è successiva alla seconda data, restituisce **0** (zero) quando la prima data corrisponde alla seconda data o restituisce un valore negativo quando la prima data è antecedente alla seconda data. | **DAYS (TODAY (), DATEVALUE( DATETIMEFORMAT( ADDDAYS(NOW(), 1), "yyyyMMdd"), "yyyyMMdd"))** restituisce **-1**. |
...
<td>Questa funzione viene eseguite come selezione in memoria. Restituisce un nuovo elenco bidimensionale che rappresenta tutti gli elementi che corrispondono a un percorso specifico. Il percorso deve essere definito come percorso di un'origine dati valido in un elemento di origine dati di un tipo di dati elenco di record. Gli elementi di dati come stringa percorso e data devono generare un errore in fase di progettazione nel generatore di espressioni ER.</td>
...
<tr>
<td>ALLITEMSQUERY (percorso)</td>
<td>Questa funzione viene eseguita come query SQL con join. Restituisce un nuovo elenco bidimensionale che rappresenta tutti gli elementi che corrispondono a un percorso specifico. Il percorso specificato deve essere definito come percorso di un'origine dati valido in un elemento di origine dati di un tipo di dati elenco di record e deve contenere almeno una relazione. Gli elementi di dati come stringa percorso e data devono generare un errore in fase di progettazione nel generatore di espressioni ER.</td>
<td>Definire le origini dati seguenti nel mapping di modello:
<ul>
<li><strong>CustInv</strong> (tipo <strong>Record di tabella</strong>), che fa riferimento alla tabella CustInvoiceTable</li> 
<li><strong>FilteredInv</strong> (tipo <strong>Campo calcolato</strong>), che contiene l'espressione <strong>FILTER (CustInv, CustInv.InvoiceAccount = &quot;US-001&quot;)</strong></li>
<li><strong>JourLines</strong> (tipo <strong>Campo calcolato</strong>), che contiene l'espressione <strong>ALLITEMSQUERY (FilteredInv.'&lt;Relations'.CustInvoiceJour.'&lt;Relations'.CustInvoiceTrans)</strong></li>
</ul>
<p>Quando si esegue il mapping di modello per chiamare l'origine dati <strong>JourLines</strong>, viene eseguita la seguente istruzione SQL:</p>
...
I campi <strong>Descrizione</strong> ed <strong>Etichetta</strong> restituiscono valori in fase esecuzione in base alle impostazioni di lingua del formato.
</td>
...
<td>Se si immette <strong>SPLIT(&quot;abc&quot; , 1)</strong> come origine dati (DS), <strong>STRINGJOIN (DS, DS.Value, &quot;-&quot;)</strong> restituisce <strong>&quot;a-b-c&quot;</strong>.</td>
</tr>
<tr>
<td>SPLITLISTBYLIMIT (elenco, valore limite, origine limite)</td>
<td>Divide l'elenco specificato in nuovo elenco di sottoelenchi e restituisce il risultato nel contenuto dell'elenco di record. Il parametro <strong>valore limite</strong> definisce il valore del limite per la divisione dell'elenco originale. Il parametro <strong>origine limite</strong> definisce il passo di cui viene incrementata la somma totale. Il limite non è applicato a un singolo articolo dell'elenco originale se l'origine limite supera il limite definito.</td>
<td>Nella figura seguente viene illustrato un formato. 
<p><a href="./media/ger-splitlistbylimit-format.png"><img src="./media/ger-splitlistbylimit-format.png" alt="Format" class="alignnone size-full wp-image-1204063" width="396" height="195" /></a></p>
<p>Nella figura seguente vengono mostrate le origini dati che vengono utilizzate per il formato.</p>
...
<tr>
<td>TEXT (input)</td>
<td>Restituisce l'input specificato dopo che è stato convertito in stringa di testo che viene formattata in base alle impostazioni locali server dell'istanza corrente dell'applicazione. Per i valori di tipo <strong>real</strong>, la conversione di stringhe è limitata a due posizioni decimali.</td>
<td>Se le impostazioni locali server dell'istanza di Finance and Operations vengono definite come <strong>EN-US</strong>, <strong>TEXT (NOW ())</strong> restituisce la data della sessione corrente dell'applicazione, 17 dicembre 2015, come stringa di testo <strong>&quot;12/17/2015 07:59:23 AM&quot;</strong>. <strong>TEXT (1/3)</strong> restituisce <strong>&quot;0.33&quot;</strong>.</td>
</tr>
<tr>
...
<ul>
<li><strong>Per le etichette di risorse della app Finance and Operations:</strong> <strong>@&quot;X&quot;</strong>, dove <strong>X</strong> è l'ID etichetta nell' Application Object Tree.</li>
...
<td>Restituisce una rappresentazione di stringa del numero specificato nel formato specificato. (Per informazioni sui formati supportati, vedere <a href="https://msdn.microsoft.com/library/dwhawy9k(v=vs.110).aspx">standard</a> e <a href="https://msdn.microsoft.com/library/0c899ak8(v=vs.110).aspx">personalizzato</a>). Il contesto in cui questa funzione viene eseguita determina la lingua utilizzata per formattare i numeri.</td>
...
<tr>
<td>NUMBERFORMAT (numero, formato, cultura)</td>
<td>Restituisce una rappresentazione stringa del numero specificato nel formato e nella cultura specificati (per informazioni sui formati supportati, vedere <a href="https://docs.microsoft.com/dotnet/standard/base-types/standard-numeric-format-strings">standard</a> e <a href="https://docs.microsoft.com/dotnet/standard/base-types/custom-numeric-format-strings">personalizzato</a>).</td>
<td><strong>NUMBERFORMAT (10/3, “F2”, "de")</strong> restituisce <strong>3,33</strong> mentre <strong>NUMBERFORMAT (10/3, “F2”, "en-us")</strong> restituisce <strong>3.33</strong>.</td>
</tr>
<tr>
...
<li><strong>Users</strong> (tipo <strong>Record di tabella</strong>), che fa riferimento alla tabella UserInfo</li>
</ul>
Quando vengono definite queste origini dati, è possibile utilizzare un'espressione quale <strong>FILTER (Users, Users.objectId = myID)</strong> per filtrare la tabella UserInfo in base al campo <strong>objectId</strong> del tipo di dati <strong>GUID</strong>.
</td>
</tr>
<tr>
<td>JSONVALUE (id, percorso)</td>
<td>Analizza i dati nel formato JSON (JavaScript Object Notation) accessibile dal percorso specificato per estrarre un valore scalare basato sull'ID specifico.</td>
<td>L'origine dati <strong>$JsonField</strong> contiene i dati seguenti nel formato JSON: <strong>{&quot;BuildNumber&quot;:&quot;7.3.1234.1&quot;, &quot;KeyThumbprint&quot;:&quot;7366E&quot;}</strong>. Per questa origine dati, </strong>JSONVALUE ( &quot;BuildNumber&quot;, $JsonField)</strong> restituisce il valore <strong>7.3.1234.1</strong> del tipo di dati <strong>Stringa</strong>.</td>
</tr>
</tbody>
</table>
...
| Funzione | descrizione | Esempio |
|----------|-------------|---------|
| CONVERTCURRENCY (importo, valuta di origine, valuta di destinazione, data, società) | Converte l'importo monetario specificato da valuta di origine specificata nella valuta di destinazione specificata utilizzando le impostazioni della società specificata alla data specificata. | **CONVERTCURRENCY (1, "EUR", "USD", TODAY(), "DEMF")** restituisce un equivalente di un euro in dollari statunitensi nella data della sessione corrente, in base alle impostazioni per la società DEMF. |
| ROUNDAMOUNT (numero, decimali, regola arrotondamento) | Arrotonda l'importo specificato al numero specificato posizioni decimali in base alla regola di arrotondamento specificata.<blockquote>[!NOTE] La regola di arrotondamento deve essere specificata come valore di enumerazione **RoundOffType**.</blockquote> | Se il parametro **model.RoundOff** è impostato su **Downward**, **ROUNDAMOUNT (1000.787, 2, model.RoundOff)** restituisce il valore **1000.78**. Se il parametro **model.RoundOff** è impostato su **Normal** o **Rounding-up**, **ROUNDAMOUNT (1000.787, 2, model.RoundOff)** restituisce il valore **1000.79**. |
...
| GetCurrentCompany () | Restituisce la rappresentazione in formato testo del codice della persona giuridica (società) a cui un utente è attualmente connesso. | **GETCURRENTCOMPANY ()** restituisce **USMF** per un utente collegato alla società **Contoso Entertainment System USA**. |
...
| FA\_BALANCE (codice del cespite, codice del modello di valore, anno di dichiarazione, data di dichiarazione) | Restituisce il contenitore dati preparati del saldo cespiti. L'anno di dichiarazione deve essere specificato come valore dell'enumerazione **AssetYear**. | **FA\_SUM ("COMP-000001", "Current", AxEnumAssetYear.ThisYear, SESSIONTODAY ())** restituisce il contenitore dati preparati dei saldi del cespite **"COMP-000001"** con il modello di valore **"Current"** sulla data della sessione corrente dell'applicazione. |
