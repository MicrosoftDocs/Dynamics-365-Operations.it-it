...
+ Per le nuove istanze di app per finanza e operazioni, la configurazione di una connessione a doppia scrittura inizia in Microsoft Dynamics Lifecycle Services (LCS). Se è disponibile una licenza per Microsoft Power Platform, si ottiene uno nuovo ambiente Dataverse se il tenant ne è privo.
+ Per le istanze esistenti di app per finanza e operazioni, la configurazione di una connessione a doppia scrittura inizia nell'ambiente Finanza e operazioni.

Prima di avviare la doppia scrittura su un'entità, è possibile eseguire una sincronizzazione iniziale per gestire i dati esistenti su entrambi i lati: delle app per finanza e operazioni e delle app Customer Engagement. È possibile saltare la sincronizzazione iniziale se non si devono sincronizzare i dati tra i due ambienti.
...
+ [Una nuova istanza di app per finanza e operazioni e una nuova istanza di app Customer Engagement](#new-new)
+ [Una nuova istanza di app per finanza e operazioni e un'istanza di app Customer Engagement esistente](#new-existing)
+ [Una nuova istanza di app per finanza e operazioni con dati e una nuova istanza di app Customer Engagement](#new-data-new)
+ [Una nuova istanza di app per finanza e operazioni con dati e un'istanza esistente di app Customer Engagement](#new-data-existing)
+ [Un'istanza di app per finanza e operazioni esistente e una nuova istanza dell'app Customer Engagement esistente](#existing-new)
+ [Un'istanza di app per finanza e operazioni esistente e un'istanza esistente dell'app Customer Engagement esistente](#existing-existing)

## <a name="a-new-finance-and-operations-app-instance-and-a-new-customer-engagement-app-instance"></a><a id="new-new"></a>Una nuova istanza dell'app per finanza e operazioni e una nuova istanza dell'app Customer Engagement

Per impostare una connessione a doppia scrittura tra una nuova istanza dell'app per finanza e operazioni che non contiene dati e una nuova istanza di un'app Customer Engagement, seguire i passaggi in [Configurazione della doppia scrittura da Lifecycle Services](lcs-setup.md). Al termine della configurazione della connessione, si verificano automaticamente le seguenti azioni:

- Viene eseguito il provisioning di un nuovo ambiente Finanza e operazioni vuoto.
...
## <a name="a-new-finance-and-operations-app-instance-and-an-existing-customer-engagement-app-instance"></a><a id="new-existing"></a>Una nuova istanza di app per finanza e operazioni e un'istanza dell'app Customer Engagement esistente

Per impostare una connessione a doppia scrittura tra una nuova istanza dell'app per finanza e operazioni che non contiene dati e un'istanza esistente di un'app Customer Engagement, seguire i passaggi in [Configurazione della doppia scrittura da Lifecycle Services](lcs-setup.md). Al termine della configurazione della connessione, si verificano automaticamente le seguenti azioni:
...
Per sincronizzare i dati Dataverse esistenti nell'app per finanza e operazioni, seguire questi passaggi.

1. Crea una nuova società nell'app per finanza e operazioni.
...
## <a name="an-existing-finance-and-operations-app-instance-and-a-new-customer-engagement-app-instance"></a><a id="existing-new"></a>Un'istanza esistente dell'app per finanza e operazioni e una nuova istanza dell'app Customer Engagement esistente

L'impostazione di una connessione a doppia scrittura tra un'istanza esistente di un'app per finanza e operazioni e una nuova istanza di un'app Customer Engagement si verifica nell'ambiente Finance and Operation.

1. [Configura la connessione dall'app per finanza e operazioni ](enable-dual-write.md).
2. Eseguire la funzionalità **Sincronizzazione iniziale** per le tabelle per cui si desidera sincronizzare i dati.

Per i collegamenti a un esempio e un approccio alternativo, consultare la sezione [Esempio](#example).

## <a name="an-existing-finance-and-operations-app-instance-and-an-existing-customer-engagement-app-instance"></a><a id="existing-existing"></a>Un'istanza di app per finanza e operazioni esistente e un'istanza esistente dell'app Customer Engagement esistente

L'impostazione di una connessione a doppia scrittura tra un'istanza esistente di un'app per finanza e operazioni e un'istanza esistente di un'app Customer Engagement si verifica nell'ambiente Finance and Operation.

1. [Configura la connessione dall'app per finanza e operazioni ](enable-dual-write.md).
2. Per sincronizzare i dati Dataverse esistenti nell'app per finanza e operazioni, [avviare](bootstrap-company-data.md) i dati Dataverse utilizzando un codice azienda ISO di tre lettere.
