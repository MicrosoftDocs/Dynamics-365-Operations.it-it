...
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="3cfb8-103">Domande frequenti sul bootstrap con i dati della società</span><span class="sxs-lookup"><span data-stu-id="3cfb8-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../../includes/banner.md)]


## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="3cfb8-104">Perché è necessario eseguire il bootstrap?</span><span class="sxs-lookup"><span data-stu-id="3cfb8-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="3cfb8-105">È possibile avere un Common Data Service esistente o un'altra istanza dell'app Dynamics 365 con dati aziendali e si desidera abilitare la connessione a doppia scrittura.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-105">You might have an existing Common Data Service or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="3cfb8-106">In questo caso, è necessario eseguire il bootstrap di Common Data Service o altri dati dell'app Dynamics 365 con le informazioni della società prima di abilitare la connessione a doppia scrittura.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-106">In this case, you need to bootstrap Common Data Service or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="3cfb8-107">Quando si deve utilizzare il bootstrap?</span><span class="sxs-lookup"><span data-stu-id="3cfb8-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="3cfb8-108">È necessario utilizzare il bootstrap prima di abilitare le mappe entità a doppia scrittura (durante il passaggio n. 5).</span><span class="sxs-lookup"><span data-stu-id="3cfb8-108">You should use bootstrapping before enabling dual-write entity maps (during step #5).</span></span>  
1. <span data-ttu-id="3cfb8-109">Per configurare la connessione di doppia scrittura tra le istanze dell'app Finance and Operations e Common Data Service o altra app Dynamics 365, accedere all'app Finance and Operations come amministratore.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-109">To setup the dual-write connection between instances of your Finance and Operations app and the Common Data Service or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="3cfb8-110">Passare al modulo **Gestione dei dati** e fare clic sul pulsante **Doppia scrittura**.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="3cfb8-111">Viene avviato l'**integratore dei dati**.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="3cfb8-112">Creare la connessione a doppia scrittura per una o più società.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="3cfb8-113">![Creare la connessione a doppia scrittura](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="3cfb8-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="3cfb8-114">Attivare la mappa dell'entità **Cdm_companies**.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-114">Enable the **Cdm_companies** entity map.</span></span> <span data-ttu-id="3cfb8-115">In tal modo si sincronizzano le società dall'app Finance and Operations in Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-115">This synchronizes companies from the Finance and Operations app to Common Data Service.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="3cfb8-116">![Attivare la mappa dell'entità](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="3cfb8-116">![Enable the entity map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="3cfb8-117">Eseguire il codice di bootstrap di esempio in Common Data Service o in un'altra istanza dell'app Dynamics 365.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-117">Run the sample bootstrapping code on the Common Data Service or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="3cfb8-118">Al termine del bootstrap e il sistema è pronto per la sincronizzazione in tempo reale, abilitare le mappe entità.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-118">When the bootstrapping is done and the system is ready for the live sync, enable the entity maps.</span></span>  

    <span data-ttu-id="3cfb8-119">L'abilitazione delle mappe di entità attiva la sincronizzazione dei dati iniziali per le mappe entità abilitate.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-119">Enabling the entity maps triggers the initial data sync for the enabled entity maps.</span></span> <span data-ttu-id="3cfb8-120">I dati corrispondenti alle società scelte sulla connessione a doppia scrittura sono sincronizzati tra l'app Finance and Operations e Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Common Data Service.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="3cfb8-121">Come si usa l'esempio di codice?</span><span class="sxs-lookup"><span data-stu-id="3cfb8-121">How to I use the code sample?</span></span>
<span data-ttu-id="3cfb8-122">Il codice di esempio è un'applicazione C # che puoi caricare in Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="3cfb8-123">Richiede dipendenze del pacchetto NuGet dall'SDK di Common Data Service che è possibile aggiornare tramite gli strumenti standard di Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-123">It takes NuGet package dependencies on the Common Data Service SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="3cfb8-124">Dopo aver decompresso e aperto la soluzione in Visual Studio e recuperato i pacchetti NuGet, cercare **TODO** nel codice.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="3cfb8-125">Ogni decisione che è necessario prendere su come si desidera avviare il bootstrap delle informazioni aziendali è annotata da un **TODO**, con un codice di esempio per un'implementazione canonica.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO**, with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="3cfb8-126">Il codice di esempio mostra solo uno dei molti modi in cui è possibile classificare i record di entità per società.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-126">The sample code shows only one of many ways you might categorize entity records by company.</span></span> <span data-ttu-id="3cfb8-127">Modificando la logica nelle sezioni **TODO**, è possibile creare la categorizzazione personalizzata.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="3cfb8-128">Cosa ci si deve aspettare?</span><span class="sxs-lookup"><span data-stu-id="3cfb8-128">What should I expect?</span></span>
<span data-ttu-id="3cfb8-129">Per impostazione predefinita, l'applicazione di esempio consente di fornire un dizionario di mappature del codice da unità aziendale a società.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="3cfb8-130">Qualsiasi entità avviata con un campo **OwningBusinessUnit** viene automaticamente impostata per utilizzare la società specificata.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="3cfb8-131">Qualsiasi entità senza un campo **OwningBusinessUnit**, ad esempio un prodotto, imposterà la società in base alla mappatura con un valore di unità di business vuoto.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="3cfb8-132">L'applicazione della console prevede un parametro, **–simulate** o **–apply**.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="3cfb8-133">Se si utilizza il parametro della riga di comando **–simulate**, i dati non vengono aggiornati.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="3cfb8-134">Solo i file **simulation_<entityname>.csv** vengono generati nella stessa directory dello strumento, uno per ciascuna entità che sarebbe stata aggiornata.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="3cfb8-135">È possibile rivedere iterativamente questi file mentre si lavora per assicurarsi che il codice aggiorni i valori dell'azienda come previsto.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="3cfb8-136">Al termine degli aggiornamenti simulati, utilizzare il parametro **–apply**.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="3cfb8-137">In tal modo vengono aggiornati tutti i record che attualmente hanno un valore aziendale errato, in batch di 1000 record alla volta (per impostazione predefinita).</span><span class="sxs-lookup"><span data-stu-id="3cfb8-137">This updates all records that currently have an incorrect company value, in batches of 1000 records at a time (by default).</span></span> <span data-ttu-id="3cfb8-138">Il codice è idempotente come indicato, il che significa che è possibile eseguirlo nuovamente e verranno aggiornate solo le società assegnate in modo errato.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="3cfb8-139">Durante l'esecuzione di **–apply**, il codice, genera i file CSV delle modifiche effettuate ovvero **applied_<entityname>.csv**.</span><span class="sxs-lookup"><span data-stu-id="3cfb8-139">When running with **–apply**, the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
