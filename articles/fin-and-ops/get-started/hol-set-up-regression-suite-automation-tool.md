...
- Comprendere il modo in cui la funzionalità Registrazione attività in Microsoft Dynamics 365 for Finance and Operations, insieme a Microsoft Dynamics Lifecycle Services e Microsoft Azure DevOps, consente di creare, configurare, eseguire e analizzare test case nonché generare report sugli stessi.
- Consentire agli utenti funzionali di registrare attività aziendali utilizzando Registrazione attività in Finance and Operations e quindi convertire le registrazioni attività in un gruppo di test automatici, senza dover scrivere codice sorgente.

## <a name="before-you-begin"></a>Prima di iniziare

### <a name="prerequisites"></a>Prerequisiti

- Un ambiente di Finance and Operations in cui viene eseguito Microsoft Dynamics 365 for Finance and Operations versione 10.0 (aprile 2019) o versione successiva è necessario per questa esercitazione. Per i clienti che utilizzano Microsoft Dynamics 365 for Finance and Operations, Enterprise Edition 7.3, è supportato anche l'aggiornamento 20 della piattaforma (PU20) o un aggiornamento successivo.
- L'utente deve disporre dei diritti di amministratore per l'ambiente di Finance and Operations.
...
## <a name="finance-and-operations-environment"></a>Ambiente di Finance and Operations

### <a name="version-requirement"></a>Requisiti di versione

È necessario un ambiente di test o sandbox di Finance and Operations in cui viene eseguita la versione 10. Per i clienti che utilizzano la versione 7.3, è supportato anche l'update 20 della piattaforma o un update successivo.

> [!NOTE]
> RSAT deve avere accesso all'ambiente di test di Finance and Operations tramite un Web browser.
...
Questo passaggio è necessario per eseguire la connessione a LCS di modo che le registrazioni attività possano essere salvate nella libreria BPM appropriata in LCS tramite il client di Finance and Operations.

1. Aprire il client di Finance and Operations.
...
> [!NOTE]
> Verificare che tutte le registrazioni attività siano avviate del dashboard principale di Finance and Operations. Fare in modo che le singole registrazioni siano brevi e focalizzarsi su un'attività aziendale, ad esempio la creazione di un ordine cliente.
...
RSAT può essere installato in qualsiasi computer in cui è in esecuzione Windows 10 e che può essere connesso all'ambiente di Finance and Operations tramite un Web browser (Internet Explorer o Google Chrome).
...
> [!NOTE]
> Se l'ambiente di Finance and Operations è un ambiente di livello 2+, l'identificazione personale del certificato deve essere aggiornata nel file wif.config per **tutti** i computer AOS per l'ambiente.
...
4. Aggiornare il file **wif.config** aggiungendo una nuova voce di autorità per il certificato e il nome dell'autorità, come illustrato nell'esempio riportato di seguito.
    ...
    > [!NOTE]
    > Se più utenti utilizzano la stessa applicazione di Finance and Operations, ogni utente deve generare identificazioni digitali distinte e ognuna di queste identificazioni deve essere aggiunta nella sezione **\<keys\>**.
...
3. Fare clic sul pulsante **Impostazioni** (il simbolo di ingranaggio) nell'angolo superiore destro e quindi, nella finestra di dialogo visualizzata, riempire i seguenti campi:
    ...
    - **Nome host** - Immettere il nome host dell'ambiente di test di Finance and Operations, ad esempio **\<myaos\>.cloudax.dynamics.com**. Non includere il prefisso **http://** o **https://**.
    - **Nome host SOAP** - Immettere il nome host SOAP dell'ambiente di test di Finance and Operations. In genere, il nome host SOAP corrisponde al nome host, ma presenta il suffisso **soap**. Ad esempio: **\<myaos\>soap.cloudax.dynamics.com**. Non includere il prefisso **http://** o **https://**.
    ...
    - **Nome utente amministratore** - Immettere l'indirizzo di posta elettronica di un utente amministratore nell'ambiente di test di Finance and Operations.
    ...
    - **Nome società** - Immettere il nome della società da utilizzare come società di Finance and Operations predefinita quando vengono creati i file di parametri di Excel. È possibile modificare la società in seguito modificando il file di parametri di Excel.
...
3. Per generare un numero di prodotto univoco per ogni esecuzione senza dover riaprire il file di parametri di Excel e aggiornare manualmente il numero di prodotto ogni volta, utilizzare la seguente formula di Excel.
    ...
    > [!NOTE]
    > Oltre alla scheda **Generale**, il file di parametri di Excel contiene una scheda dati per ogni pagina modulo di Finance and Operations visitata dal test case.
