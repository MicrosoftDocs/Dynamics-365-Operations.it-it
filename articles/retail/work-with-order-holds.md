...
# <a name="work-with-call-center-order-holds"></a>Utilizzo delle sospensioni ordine servizio clienti
...
Per creare un set facoltativo di codici motivo della sospensione, passare a **Retail** \> **Impostazione canale** \> **Codici informativi**. Questi codici informativi possono essere utilizzati come codice motivo secondario per definire ulteriormente il codice di sospensione principale. Selezionare **Nuovo** per creare un set di codici motivo e quindi selezionare **Sottocodici** per definire l'elenco dei motivi aggiuntivi. Per collegare tutti i codici informativi che si definiscono nel canale del servizio clienti, passare a **Retail** \> **Canali** \> **Servizi clienti** \> **Tutti i servizi clienti**. Nella Scheda dettaglio **Generale** impostare il campo **Codice sospensione**.

## <a name="putting-orders-on-hold"></a>Ordini in attesa

Gli ordini che gli utenti del servizio clienti creano nel programma Retail di back-office possono essere messi in attesa manualmente o automaticamente in situazioni specifiche.
...
Se il flag **Attiva completamento ordine** è attivato nel canale di servizio clienti, non è necessario che venga applicato il pagamento a un ordine messo in attesa. Di contro, per un ordine cliente che non viene messo in attesa, gli utenti non possono uscire dalla pagina di registrazione dell'ordine cliente senza applicare il pagamento. Naturalmente, il pagamento verrà richiesto prima del rilascio della sospensione dell'ordine.
...
Per visualizzare tutti gli ordini che sono stati messi in attesa manualmente o automaticamente, passare a **Retail** \> **Clienti** \> **Sospensioni ordine**.
