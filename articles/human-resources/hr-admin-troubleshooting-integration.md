...
In questo articolo vengono fornite le risposte a domande comuni relative ai dati che vengono sincronizzati durante l'integrazione di Dynamics 365 Human Resources con Dynamics 365 Finance.
...
N. Se si modifica l'utente dell'applicazione Human Resources, l'integrazione tra Human Resources e Dataverse potrebbe non riuscire. La tabella seguente mostra le impostazioni predefinite per l'utente dell'applicazione Talent.
...
## <a name="why-dont-i-see-any-data-synced-to-dataverse"></a>Perché non vedo i dati sincronizzati in Dataverse?

Per impostazione predefinita, l'integrazione Dataverse è disattivata nei nuovi ambienti che non includono i dati dimostrativi forniti. Per impostazione predefinita, è abilitata nei nuovi ambienti che contengono i dati dimostrativi e la sincronizzazione dei dati inizia quando l'ambiente viene sottoposto al provisioning. Una volta che l'ambiente è pronto per sincronizzare i dati è possibile attivare l'integrazione. Per ulteriori informazioni, vedere [Configurare l'integrazione di Dataverse](hr-admin-integration-common-data-service.md).
...
Le dimensioni finanziarie non sono attualmente incluse in Dataverse e di conseguenza non fanno parte del modello predefinito. Questa entità è pianificata, ma al momento non è disponibile alcuna data di rilascio.
...
![Mappare le dimensioni finanziarie.](media/MapFinancialDimensions.png)
...
I campi possono essere aggiunti o rimossi dall'attività di integrazione. Non tutti i campi dati esistenti nella tabella di Dataverse verranno popolati a partire da Human Resources.
Ulteriori dati possono essere popolati via Power Apps.

![Aggiungere o rimuovere campi in/da un'attività di integrazione.](media/SpecifyFieldsIncludedInIntegration.png)
...
La prima esecuzione dell'integratore di dati è sempre un'esecuzione completa. Le esecuzioni successive sono basate sul rilevamento delle modifiche. In caso di esecuzione con errori, estrae i record nell'ambito dell'esecuzione e invia le modifiche più recenti da Dataverse.
...
Sì, dei campi possono essere aggiunti a o rimossi dal modello esistente. Il modello può essere modificato affinché includa ulteriori dati da altre tabelle di Dataverse. L'entità deve essere in Dataverse affinché venga inclusa nel modello. 
...
L'integrazione con Finance è un processo in due fasi. La prima consiste nel verificare che i dati di Human Resources vengano aggiornati e siano disponibili in Dataverse. Questa fase è una sincronizzazione quasi in tempo reale e può essere verificata in Power Apps esaminando i dati nelle tabelle di dati.

![Dati in Dataverse.](media/DataInCDS.png)

Se i dati non sono visualizzati come previsto in Dataverse, verificare che l'entità sia supportata nell'integrazione. Per includere ulteriori dati in Dataverse, sarà necessaria una modifica lato Microsoft.

Se l'entità è supportata e i dati sono disponibili in Dataverse, verificare che il mapping sia corretto nell'integratore di dati. Se il mapping dell'integratore sembra corretto, verificare che i processi di gestione dei dati siano stati eseguiti correttamente. È possibile che si verifichino degli errori durante l'esecuzione dei processi batch. Per ulteriori informazioni sulla gestione dei dati, vedere [Gestione dei dati](/dynamics365/unified-operations/dev-itpro/data-entities/data-entities-data-packages?toc=%2ffin-and-ops%2ftoc.json).
...
Quando si creano delle connessioni, scegliere Dynamics 365 Finance e Dataverse.
...
![Mapping campi mancanti.](media/MissingFieldMapping.png)

## <a name="additional-resources"></a>Risorse aggiuntive

- Integratore di dati (ID): 

  - [Integrazione dei dati in Microsoft Dataverse](/powerapps/administrator/data-integrator)

  - [Gestione e risoluzione degli errori nell'integratore di dati](/powerapps/administrator/data-integrator-error-management)

  - [Rispondere alle richieste DSR per log generati dal sistema in Power Apps, Microsoft Power Automate e Dataverse](/powerapps/administrator/powerapps-gdpr-dsr-guide-systemlogs)

- Gestione dei dati:

  - [Gestione dei dati](/dynamics365/unified-operations/dev-itpro/data-entities/data-entities-data-packages?toc=%2ffin-and-ops%2ftoc.json)
