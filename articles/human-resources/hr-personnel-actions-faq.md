...
description: In questo argomento vi sono risposte alle domande che potrebbe essere poste se l'organizzazione utilizza le azioni dipendente.
author: twheeloc
ms.date: 10/28/2021
ms.topic: article
ms.prod: ''
...
In questo argomento vi sono risposte alle domande che potrebbe essere poste se l'organizzazione utilizza le azioni dipendente. Le azioni dipendente sono passaggi aggiuntivi da completare quando si eseguono attività correlate al personale. 

Esempi di attività che potrebbero richiedere azioni del personale sono:
 - Quando crei nuove posizioni. 
 - Modifica i valori di posizione esistenti. 
 - Assumi nuovi lavoratori. 
 - Trasferisci lavoratori. 
 - Cambia retribuzione lavoratore. 
 - Modifica assegnazioni di posizione. 
 - Termina rapporto con lavoratori.

> [!NOTE]
> Le azioni dipendente sono disponibili solo se i campi **Attiva azioni lavoratore** e **Attiva azioni posizione** sono stati impostati su **Sì**, nella scheda **Azioni dipendente** della pagina **Parametri condivisi risorse umane**. 
...
- Azione di **posizione**: un'azione di posizione viene eseguita nelle posizioni esistenti o nelle nuove posizioni. Ad esempio, un'azione di posizione può essere obbligatoria se si modifica un valore in una posizione esistente o se si crea una nuova posizione stagionale. 
...
> 1. Nella pagina **Assegnazioni di posizione lavoratore**, fare clic sul campo **Posizione**.  
...
## <a name="why-cant-i-change-the-effective-date-on-the-header-of-the-worker-action-page"></a>Per quale motivo non è possibile modificare la data di validità nell'intestazione della pagina Azione lavoratore?
