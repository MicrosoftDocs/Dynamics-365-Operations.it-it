---
title: Panoramica processi di produzione
description: L&quot;articolo viene fornita una panoramica dei processi di produzione. Vengono descritte le varie fasi degli ordini di produzione, ordini lotto e kanban, dalla creazione dell&quot;ordine e la chiusura del periodo finanziario.
author: YuyuScheller
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
# <a name="production-process-overview"></a>Panoramica processi di produzione

L'articolo viene fornita una panoramica dei processi di produzione. Vengono descritte le varie fasi degli ordini di produzione, ordini lotto e kanban, dalla creazione dell'ordine e la chiusura del periodo finanziario. 
...
** Controllo produzione ** il modulo agli altri moduli, ad esempio ** Gestione delle informazioni sul prodotto, ** ** Gestione articoli, ** ** contabilità generale, ** ** gestione magazzino, ** ** Contabilità progetti e ** ** Amministrazione organizzazione **. Grazie a tale integrazione, viene garantito il supporto del flusso di informazioni necessario per completare la produzione di un articolo finito.  

Il processo di produzione in genere è influenzato dalla contabilità industriale e dai metodi di valutazione del magazzino scelti per un processo di produzione specifico. Dynamics 365 per le operazioni supporta sia il costo effettivo (First In, First Out) in \[\]; (Last In, First Out) \[\]LIFO; media mobile; e media ponderata periodico) e metodi di costo standard. La produzione snella è implementata in base al principio di determinazione costi di tipo backflush.  
...
Le topologie di produzione e i prodotti diversi richiedono l'applicazione di tipi di ordine diversi. Dynamics 365 per le operazioni può applicare i diversi tipi di ordine in un misto. In altre parole, tutti i tipi di ordine possono verificarsi durante il processo di produzione end-to-end di un prodotto finito.
...
2.  **Stimato**: è possibile calcolare le stime per il consumo di materiali e risorse. La stima genera operazioni di magazzino per le materie prime con stato **In ordinazione**. Le entrate dei prodotti, sotto/co-prodotti principali sono generano quando gli ordini di produzione o gli ordini lotto vengono stimati. Se la DBA contiene le righe di rifornimento ** cavigliato ** tipo, ordini fornitori per i materiali o servizi in conto lavoro dell'operazione e vengono generate cavigliate all'ordine di produzione o all'ordine lotto. Gli articoli o gli ordini vengono prenotati in base alla strategia di prenotazione dell'ordine di produzione e il prezzo dei prodotti finiti viene calcolato in base alle impostazioni dei parametri.
