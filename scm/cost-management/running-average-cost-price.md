...
Il sistema stima il prezzo di costo medio corrente di un articolo utilizzando la seguente formula: Prezzo stimato = (Importo fisico + Importo finanziario) ÷ (Quantità fisica + Quantità finanziaria)
...
\*Numeratore = (Importo fisico + Importo finanziario) \*\*Denominatore = (Quantità fisica + Quantità finanziaria) **Nota**: se l'opzione **Includi valore fisico** non è selezionata per un articolo, il sistema utilizza 0 (zero) sia per l'importo fisico che per la quantità fisica. Per informazioni su questa opzione, vedere [Includi valore fisico](include-physical-value.md).
