---
title: "Novità o variabile in Dynamics AX 7.0 (febbraio 2016)"
...
# <a name="whats-new-or-changed-in-dynamics-ax-70-february-2016"></a>Novità o variabile in Dynamics AX 7.0 (febbraio 2016)
...
| Automatizzare la compilazione, il test e la distribuzione.                                                                            | Parzialmente disponibile                                                                                                                                                                                | Distribuire la topologia di sviluppo utilizzando Developer and Build VM. Configurare automaticamente Build VM per individuare e sviluppare i moduli di Visual Studio Online (VSO) e per l'esecuzione di test. La compilazione e i riferimenti al modulo di\# e C X++ sono supportati. | Aumenta la produttività di sviluppo riducendo il costo e lo sforzo per test e convalide.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| Approfitta del connettore aziendale per creare gli scenari di integrazione del supporto e di logica di business.         | Il Business Connector è disponibile per chiamare il codice X++ dal codice gestito. Si consiglia di utilizzare il Business Connector per creare solo la regola business C in\#, non per scenari di integrazione. | Il connettore commerciale non è più supportato. Il requisito di creazione viene fornito dal fatto che X++ viene compilato nel codice gestito. Di conseguenza, Interop è più semplice. Gli scenari di integrazione vengono soddisfatti utilizzando OData.       | Non è possibile utilizzare il connettore aziendale d'ora in avanti.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
<td>Le opzioni di report sono state aggiunte, in modo che i filtri possono essere applicati durante la visualizzazione di un report. Un nuovo report verrà generato in base ai filtri.</td>
...
<td>Analisi dei dati finanziari utilizzando al contenuto di potenza delle prestazioni finanziarie di visualizzare" BI.</td>
<td>Non disponibile</td>
...
<td>Nella Guida della versione corrente di attività di Dynamics AX sono visualizzate guide di attività archiviate in Microsoft Dynamics Lifecycle Services (LCS) BPM. Nella Guida sono inoltre visualizzati gli argomenti inclusi nella wiki di Microsoft. Per ulteriori informazioni, vedere [la Guida di Microsoft Dynamics AX - a partire help-overview.md] () e [nuove consente di attività disponibili (febbraio 2016)](new-task-guides-available-february-2016.md).</td>
<td>La guida attività fornisce un'esperienza controllata, guidata e interattiva che guida l'utente attraverso i passaggi di un'attività o un processo aziendale. È possibile scaricare e personalizzare le guide attività fornite da Microsoft. Il wiki fornisce un modo più rapido e flessibile per creare, distribuire e aggiornare la documentazione del prodotto. Di conseguenza, aiuta a garantire l'accesso alle ultime informazioni tecniche.</td>
</tr>
</tbody>
</table>
...
<td>La funzionalità viene implementata come configurazione elettronica di reporting. La funzionalità è disponibile per la Germania e l'Austria.</td>
...
<td>Elenco saldi con il report dei conti totali di gruppo è implementato come Report SSRS (LedgerAccountSum\_franco).</td>
...
<td>Nell'elenco separato dei codici voce doganale Intrastat verrà unito con la gerarchia dei prodotti di tipo codice voce doganale.</td>
...
<td>Codice voce doganale Intrastat è un campo di testo per identificare le unità supplementari e ** prodotto ** la scheda è un campo per identificare la quantità di unità supplementari in chilogrammi.</td>
...
<tr class="odd">
<td>Analizzare ai dati del canale utilizzando al contenuto di potenza delle prestazioni di Manica di vendita al dettaglio di visualizzare" BI.</td>
<td>Non disponibile</td>
...
<li>Offre pacchi alternativi "" pronti di consegna, come combinazione sito di immagazzinamento o come/modalità di trasporto/varianti, in base ai criteri di consegna più rapida (disponibile data precedente) dall'utente possa effettuare una selezione.</li>
