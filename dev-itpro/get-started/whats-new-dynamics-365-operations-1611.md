...
| Creare e gestire le spese non collegate a una nota spese.    | L'utente può ora creare spese e gestirle, ad esempio allegando o dettagliando  ricevute o assegnando ospiti) senza dover prima creare una nota spese. Una o più spese possono essere selezionate e aggiunte a una nuova nota spese, in modo che possono essere inviate per l'approvazione. Una nuova pagina per le spese di manutenzione è disponibile ** Gestione spese ** &gt; ** recuperare le spese ** &gt; ** ** spese.                                                                                                                                                                                                                                                       |
...
| Utilizzare i periodi di revisione per definire l'intervallo temporale per la revisione.                                    | È possibile definire un periodo per una revisione delle prestazioni e le date di inizio e di fine della revisione vengono immesse automaticamente. Tuttavia, è possibile modificare tali date predefinite secondo le esigenze.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Accesso a cinque nuovi di potenza colli del contenuto BI per le risorse umane                                     | I nuovi contenuto colli consentono ai responsabili delle organizzazioni Risorse umane e delle risorse umane di analizzare le seguenti operazioni: Nella forza lavoro • Ripartizioni dati per la validità, il sesso e lo stato civile • Confrontare l'anno di percentuali di logoramento nell'anno e visualizzare l'elenco dei dipendenti di motivi Dare per consentire all'organizzazione • Consente di visualizzare un elenco di posizioni aperte nonché un confronto tra aperto e la retribuzione chiusa e i benefit di posizioni • Confronto dei dipendenti orari e stipendiati • Visualizza il numero di dipendenti dai benefit disponibili • Visualizza i dipendenti indipendentemente dalla selezione di impiego • Analizzare i candidati in base al numero dei candidati, da cui provengano e di posizioni che si applicano alla formazione organizzativo • Tipi della registrazione per ubicazione • Presenza di Couse dallo stato • L'elenco di dipendenti registrate per le competenze e lo sviluppo dei dipendenti di corsi • Elenco delle competenze impostate dai dipendenti • Scomposizione di tipi di competenze dal membro del team |
...
<td>Banca LC Perc. Formato di allocazione all'Arabia Saudita</td>
</tr>
<tr class="odd">
...
### <a name="personalized-product-recommendations"></a>Suggerimenti personali del prodotto

| Operazioni che è possibile effettuare                                                                                                                                                                                                                                                                        | Perché questo è importante                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Consente di visualizzare i suggerimenti personali del prodotto in più touchpoints sul POS (POS) determinare il significato di un cliente potrebbe essere interessato in base al storico acquisti, gli articoli nell'elenco di obiettivi e gli articoli che altri clienti essi acquistati in linea e le quantità richieste di mattone-e- mortaio | Per i rivenditori ai cataloghi grandi dimensioni, i suggerimenti personali per semplificare il cliente con l'importo rilevamento del prodotto e autorizzano partner di punti vendita con clienteling intelligente. Montrando i prodotti mirati agli interessi di un cliente e alle di acquisto, i suggerimenti del prodotto sono disponibili i rivenditori a upsell e grado della ritenuta cliente. In Dynamics 365 per le operazioni al dettaglio, suggerimenti dei prodotti sono alimentati ai servizi eapprendimento automatico conoscitivi di Microsoft Azure. |
...
| Scoprire ulteriori informazioni sul supporto di spedizione in Dynamics 365 for Operations                                                                                                         | Per ulteriori informazioni sul supporto per i processi di spedizione, vedere spedizione [] (/dynamics365/operations/scm/inventory/consignment), [] spedizione dell'impostazione (/dynamics365/operations/scm/inventory/setup-consignment), [creare un ordine di rifornimento di spedizione della Guida (attività)](http://ax.help.dynamics.com/en/wiki/create-a-consignment-replenishment-order/) e [modificare la proprietà delle scorte di spedizione in base alla richiesta di produzione (Guida di attività)](http://ax.help.dynamics.com/en/wiki/change-the-ownership-of-consignment-inventory-based-on-production-demand/).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
<tbody>
<tr class="odd">
<td>Definire, direttamente nelle righe di ordine cliente, l'origine della domanda e come deve essere approvvigionata. <strong>Esempio</strong> Creare una riga ordine cliente e la consegna diretta come tipo di consegna. Il fornitore principale viene aggiunto alla riga di ordine cliente come punto di approvvigionamento.</td>
...
<tr class="even">
<td>Creare una promessa di ordine direttamente dalle righe di ordine cliente. <strong>Esempio</strong> Creare una riga ordine cliente che è originaria da un fornitore interaziendale. Lasciare la riga. Le date di consegna vengono calcolate in base alla disponibilità nella società di approvvigionamento.</td>
