...
-   L'etichetta deve iniziare con un carattere alfabetico (a-z o A-Z) e può essere una qualsiasi combinazione di numeri e lettere fino a 16 caratteri. ** Nota: ** Un'etichetta può includere il carattere di sottolineatura (\_), ma nessun altro carattere speciale.
...
La cella **Codice formato** offre una selezione di opzioni preformattate per il contenuto della riga. Se la cella **Codice formato** è vuota, la riga viene interpretata come riga di dettaglio di dati finanziari. **Nota:** se un report contiene righe di formattazione non di importo correlate a righe di importo che sono state eliminate ad esempio, a causa di saldi a zero, è possibile utilizzare la colonna **Unità/Righe/Formule correlate** per impedire la stampa delle righe del formato e del titolo.
...
3.  Selezionare un codice formato nell'elenco. Nella seguente tabella sono descritti i codici formato e le relative azioni.
    ...
    | TOT                           | Totale                              | Identifica una riga che utilizza gli operatori matematici nella colonna **Unità/Righe/Formule correlate**. I totali includono operatori semplici, ad esempio ** **+ o-** **.                                                      |
    | CAL                           | Calcolo                        | Identifica una riga che utilizza gli operatori matematici nella colonna **Unità/Righe/Formule correlate**. I calcoli contengono operatori complessi, ad esempio ** **+,-** **, ** **\*,/** ** e ** IF/THEN/ELSE ** rendiconti. |
...
Nel seguente esempio, Phyllis desidera impedire la stampa dell'intestazione e dei caratteri di sottolineatura nella riga **Cassa totale** del report perché non c'è stata attività in nessun conto di cassa. Di conseguenza, nella riga 220 (in, mentre---** ** il codice di formato è, corrisponde a una riga di formattazione), ** Formule/Righe/Unità correlate ** nella cella, immettere 250 ** **, ovvero il codice di riga della riga di importo che desidera omettere. ![[] (RelatedRowsRowDefinition. /media/relatedrowsrowdefinition-1024x144.png)](. /media/relatedrowsrowdefinition.png)
...
6.  Nel menu **File**, fare clic su **Salva con nome** per salvare la definizione di colonna con un nuovo nome. ** Aggiungono CBR ** al nome di file corrente, ad esempio ** CAGNACCIO da\_\_CBR **). La definizione di colonna è nella definizione di colonna della riga di base.
...
4.  Nella cella **Restrizione colonna**, immettere la lettera della colonna del report da utilizzare per l'ordinamento. **Nota:** includere solo le righe di importo nel calcolo di ordinamento.

### <a name="examples-of-ascending-and-descending-column-values"></a>Esempi di valori di colonna crescenti e decrescenti

Nel seguente esempio, i valori della colonna D del report verranno elencati in ordine crescente per le righe da 160 a 490. Inoltre, i valori assoluti della colonna G del report verranno elencati in ordine decrescente per le righe da 610 a 940.
...
**Per le sette mesi che terminano il 31 luglio 2013**
...
**Ordinato per scostamento assoluto da inizio anno in ordine decrescente**
...
-   **Testo di sostituzione valore zero** – Il testo da includere nel report quando un importo è 0 (zero). Questo testo verrà visualizzato come ultima riga nell'area **Esempio**. **Nota:** se la stampa è eliminata per i valori zero o nessuna attività del periodo, il testo viene eliminato.
...
Il tipo deve rappresentare il valore positivo e quindi il valore negativo. In genere, si immette un formato simile che differenzia i valori positivi e i valori negativi. Ad esempio, specificare che sia valori positivi negativi che hanno due cifre decimali, ma i valori negativi vengono visualizzati tra parentesi, immettere 0.00; **(0.00) **. Nella seguente tabella vengono indici i formati personalizzati da utilizzare per controllare il formato dei valori. Tutti gli esempi cominciano dal valore 1234,56.
...
5.  In **Attributi**, seguire questi passaggi per aggiungere una voce per ogni attributo che deve essere incluso nel codice di riga:
    1.  Fare doppio clic sulla cella **Attributo** e selezionare il nome di un attributo. ** Avvertenza: ** Sostituire il cancelletto (\#) con un valore numerico.
...
Quando si seleziona un conto specifico, Progettazione report in genere combina il conto e gli anni fiscali, i periodi e altre informazioni specificate nella definizione di colonna. È possibile utilizzare informazioni diverse, ad esempio periodi fiscali diversi, per righe specifiche. Nella seguente tabella vengono indicati i modificatori di conto disponibili. Sostituire il cancelletto (\#) con un valore a cui è uguale o inferiore al numero di periodi di un anno fiscale.

| Modificatore di conto | Cosa viene stampato                                                                           |
|------------------|-------------------------------------------------------------------------------------------|
| /BB              | Il saldo iniziale per un conto.                                                     |
| /\#              | Il saldo per il periodo specificato.                                                     |
| /-\#             | Saldo del periodo da \# periodi prima di quello corrente.                  |
| /+\#             | Saldo del periodo da \# periodi dopo quello corrente.                   |
| /C               | Il saldo del periodo corrente.                                                       |
| /C-\#            | Saldo del periodo da \# periodi prima di quello corrente.                  |
| /C +\#            | Saldo del periodo da \# periodi dopo quello corrente.                   |
| /Y               | Il saldo da inizio anno del periodo corrente.                                      |
| /Y-\#            | Saldo da inizio anno con il periodo di tempo \# periodi prima di quello corrente. |
| /Y +\#            | Saldo da inizio anno con il periodo di tempo \# periodi dopo quello corrente.  |

### <a name="book-code-modifiers"></a>Modificatori di codice registro

È possibile limitare una riga a un codice registro esistente. La definizione di colonna deve includere almeno una colonna **FD** con un codice registro. **Nota:** la restrizione del codice registro per una riga sostituisce le restrizioni del codice registro nella definizione di colonna per la riga.
...
5.  ** Da ** nel campo, immettere la dimensione iniziale, fare clic su! [Esplorare] (https://i-technet.sec.s-msft.com/dynimg/IC679490.gif "selezionare il") individuare le dimensioni disponibili. Per immettere un intervallo di dimensioni, immettere la dimensione finale nel campo **A**.
...
Quando si immette un valore di segmento di codice conto naturale ** ** dimensioni nella finestra di dialogo, è possibile inserire un carattere jolly (? o \*) in qualsiasi posizione di un segmento. In Progettazione report verranno automaticamente recuperati tutti i valori per le posizioni definite senza considerare i caratteri jolly. Ad esempio, la definizione di riga contiene solo i valori di segmenti naturali e i segmenti naturali hanno quattro caratteri. ** Il valore 6??? ** in una riga, chiedere a Progettazione report per includere tutti i conti con un valore di segmento di codice conto naturale che inizia con 6. ** Se si immette 6 **\*, gli stessi risultati vengono restituiti, ma i risultati inoltre inclusi i valori di variabili larghezza, come ** ** 60 e 600000 ** **. Progettazione report sostituisce ogni carattere jolly (?) con l'intervallo completo di valori possibili che includono lettere e caratteri speciali. Ad esempio, nell'intervallo compreso tra **12?0** e **12?4**, il carattere jolly in **12?0** viene sostituito con il valore minimo del set di caratteri e il carattere jolly in **12?4** viene sostituito con il valore massimo del set di caratteri. **Nota:** è consigliabile evitare di utilizzare i caratteri jolly per i conti iniziali e finali negli intervalli. Se si utilizzano i caratteri jolly nel conto iniziale o finale, è possibile che i risultati siano imprevisti.

### <a name="single-segment-or-single-dimension-ranges"></a>Intervalli di dimensione singola o segmento singolo

È possibile specificare un intervallo di valori di dimensione o di segmento. Il vantaggio di specificare un intervallo consiste nel fatto che non si deve aggiornare la definizione di riga ogni volta che un nuovo valore di dimensione o di segmento viene aggiunto ai dati finanziari. Ad esempio, l'intervallo ** 6100:6900 di\]\[** +Conto= tirate i valori dai conti da 6100 a 6900 nell'importo della riga. Quando un intervallo include un carattere jolly (?), Progettazione report non valuta l'intervallo sulla base di un carattere alla volta. I limiti minimo e massimo dell'intervallo sono determinati e quindi i valori finali e tutti i valori tra loro sono inclusi. ** Nota: ** In Progettazione report non è possibile selezionare i conti, le dimensioni, ovvero i campi dal sistema Microsoft Dynamics ERP che includono eventuali di caratteri riservati seguenti: e, in \*, \[, \], {, o}. È possibile aggiungere una e commerciale (&) solo quando vengono automaticamente create le definizioni di riga utilizzando la finestra di dialogo **Inserisci righe da dimensioni**.

### <a name="multiple-segment-or-multiple-dimension-ranges"></a>Intervalli di più dimensioni o segmenti

Quando si immette un intervallo utilizzando combinazioni di più valori di dimensione, il confronto viene eseguito su A. \ \ dimensioni finanziarie base di dimensione-da- dimensione. Il confronto dell'intervallo non può essere eseguito in base a carattere alla volta o in base a un segmento parziale. Ad esempio, l'intervallo ** 5000:6000\], 1000:2000, center=\]a\[00 di\]+Conto= di\[Reparto= **\[include solo i conti corrispondenti a ogni segmento. In questo esempio, la prima dimensione deve essere compresa nell'intervallo da 5000 a 6000, la seconda dimensione deve essere compresa nell'intervallo da 1000 fino al 2000 e l'ultima dimensione deve essere 00. Ad esempio, +Conto= **\[5100\], Reparto=\[1100\], center= di\[01 **\]non viene incluso nel report, perché l'ultimo segmento è rientrano nell'intervallo specificato. Se un valore di segmento include spazi, racchiudere tale valore tra parentesi quadre (\[ ). \] I valori seguenti sono validi per un segmento di quattro caratteri: **\[ 234\], \[123 \], \[1 ** 34\]. I valori di dimensione devono essere racchiusi tra parentesi quadre (\[ \]) e Progettazione report aggiungere le parentesi per l'utente. Quando un intervallo di più segmenti o dimensioni valori di segmento include caratteri jolly (? o \*), vengono individuati i valori minimo e massimo dell'intero con più segmenti o intervallo di più segmenti dimensione vengono determinati quindi i valori finali e tutti i valori tra esse inclusi. Se l'intervallo è ampio, ad esempio l'intervallo di tutti i conti compresi tra 40000 e 99999, è necessario specificare un conto di inizio e un conto di fine validi se possibile. ** Nota: ** In Progettazione report non è possibile selezionare i conti, le dimensioni, ovvero i campi dal sistema Microsoft Dynamics ERP che includono eventuali di caratteri riservati seguenti: e, in \*, \[, \], {, o}. È possibile aggiungere una e commerciale (&) solo quando vengono automaticamente create le definizioni di riga utilizzando la finestra di dialogo **Inserisci righe da dimensioni**.
...
|-----------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------|
| Aggiungere due conti completi.                                                       | +Divisione=\[000\], Conto=\[1205\], Reparto=\[00 +Divisione=\]\[100\], Conto=\[1205\], Reparto=\[00\] |
| Aggiungere due valori di segmento.                                                                 | +Conto=\[\]1205 +Conto=\[\]1210                                                                           |
| Aggiungere i valori di segmento includendo i caratteri jolly.                                    | +Conto=\[120? +Conto=\[11??\]                                                                             |
| Aggiungere un intervallo di conti completi.                                                | 000:100\], Conto=\[1205\], Reparto=\[00\]+Divisione= di\[                                                   |
| Aggiungere un intervallo di valori di segmento.                                                          | 1200:1205\]+Conto= di\[                                                                                       |
| Aggiungere un intervallo di valori di segmento includendo i caratteri jolly.                         | +Conto=\[120?: 130?\]                                                                                       |
| Sottrarre un conto completo da un altro conto completo.              | +Divisione=\[000\], Conto=\[1205\], Reparto=\[00\]- Divisione=\[100\], Conto=\[1205\], Reparto=\[00\] |
| Sottrarre un valore di segmento da un altro valore di segmento.                                  | +Conto=\[\]1205 - Conto=\[\]1210                                                                           |
| Sottrarre un valore di segmento che include un carattere jolly da un altro valore di segmento. | +Conto=\[\]1200 - Conto=\[11??\]                                                                           |
| Sottrarre un intervallo di conti completi.                                           | 000:100 A\], 1200:1205,\]00:01\]Divisione= di\[di\[Conto= Reparto= di\[                                           |
| Sottrarre un intervallo di valori di segmento.                                                     | 1200:1205 - Conto= di\]\[                                                                                       |
| Sottrarre un intervallo di valori di segmento includendo i caratteri jolly.                    | - Conto=\[120?: 130?\]                                                                                       |

Sebbene sia possibile modificare direttamente i conti, è possibile anche utilizzare la finestra di dialogo **Dimensioni** per applicare la formattazione corretta ai collegamenti dei dati finanziari. Qualsiasi valore può includere caratteri jolly (? \* o). Tuttavia, Progettazione report non è possibile selezionare i conti, le dimensioni, ovvero i campi dal sistema Microsoft Dynamics ERP che includono eventuali di caratteri riservati seguenti: e, in \*, \[, \], {, o}. **Nota:** per sottrarre i valori, è necessario racchiudere i valori tra parentesi. Ad esempio, si immette 450? **- (4509) **, viene visualizzato come ** +Conto=\[4509\]- Conto=\[450? **\] e si si indica a Progettazione report sottrarre l'importo per il segmento di conto 4509 l'importo per qualsiasi segmento di conto che inizia con 450.
...
3.  Nella prima riga della finestra di dialogo **Dimensioni**, effettuare le operazioni seguenti:
    ...
    2.  Fare doppio clic su ** operatore +/- ** la cella e selezionare il segno più (**+**) o (**-**) l'operatore negativo da applicare a uno o più valori o set di dimensioni nella riga.
...
**Nota:** l'operatore si applica a tutte le dimensioni della riga.
...
|---------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Utente o intervallo | ** Da ** nel campo, il nome di un conto, fare clic su ** individuare ** il pulsante! [Esplorare] (https://i-technet.sec.s-msft.com/dynimg/IC679490.gif "selezionare il") eseguire una ricerca per il conto. Per selezionare un intervallo, immettere o individuare un valore nel campo **A**.                                             |
| Set di valori di dimensione | Nel campo **Nome** immettere il nome di un set di valori di dimensione. Per creare, modificare, copiare o eliminare un set, fare clic su **Gestire set di valori di dimensione**. ** Formula ** il campo viene inserito la formula ** collegamento alle dimensioni finanziarie ** della cella per questo valore di dimensione nella definizione di riga. |
...
5.  Nella finestra di dialogo **Modifica**, modificare i valori di formula da includere nel set. **Nota:** se si aggiungono nuovi conti o dimensioni, assicurarsi di modificare i set esistenti di valori di dimensione per includere le modifiche.
